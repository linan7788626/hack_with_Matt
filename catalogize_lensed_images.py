...
def sersic_L_Rcut(Ieff,Reff,ndex,Rcut=100):
    ...
    #res = 2.0*np.pi*Ieff*Reff**2.0*ndex*(np.e**bn/(bn**(2.0*ndex)))*spf.gamma(2.0*ndex)
    res = 2.0*np.pi*Ieff*Reff**2.0*ndex*(np.e**bn/(bn**(2.0*ndex)))*spf.gammainc(2.0*ndex,xtmp)*spf.gamma(2.0*ndex)
    return res

def sersic_mag_in_Rcut(Ieff,Reff,ndex,Rcut=100): # g band
    mueff = 21.572+5.12-2.5*np.log10(Ieff)
    bn = 2.0*ndex-1/3.0+0.009876/ndex
    xtmp = bn*(Rcut)**(1.0/ndex)
    res = mueff-5.0*np.log10(Reff)-2.5*np.log10(2.0*np.pi*ndex*(np.e**bn/(bn**(2.0*ndex)))*spf.gammainc(2.0*ndex,xtmp)*spf.gamma(2.0*ndex))
    return res
...
def producing_lensed_images(xi1,xi2,source_cat,lens_cat):
    ...
    ql   = lens_cat[2]
    rlc  = 0.0
    rle  = re_sv(lens_cat[3],lens_cat[5],source_cat[6])       #Einstein radius of lens Arc Seconds.
...
if __name__ == '__main__':
    ...
    source_cat = [0.1, 0.2, 26, 0.1, 0.2, 35, 2.0]
    ...
    Dl_s = p13.luminosity_distance(2.0).value
    Da_s = p13.angular_diameter_distance(2.0).value
    Reff_arc = np.sqrt(0.1*0.2)
    Reff = Reff_arc*Da_s/apr*1e6 # pc
    ndex = 4.0
    Ieff = sersic_mag_tot_to_Ieff(24,Reff,ndex,2.0)
    #print sersic_mag_in_Rcut(Ieff,Reff,ndex)
    ...
    print mueff
    print Ieff2, Ieff, Reff, Reff_arc
    print Ltmp, Ltot
